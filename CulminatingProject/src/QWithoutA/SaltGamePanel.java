...
//			+ "If you happen to die, you will have to restart your journey.", "Welcome", JOptionPane.INFORMATION_MESSAGE);
	...
	public SaltGamePanel(){
		...
		ground.get(0).setWidth(ground.get(0).getWidth());
		...
		mPlat.add(new Platform(200, 300, 100, 350, 0, height));
		mPlat.get(0).setXSpeed(2);
		//adds a platform that moves on the y axis
		mPlat.add(new Platform(600, 350, 0, width, 300, 400));
		mPlat.get(1).setYSpeed(2);
				...
				if(saltBalls.size() > 0){
					...
					for(int i = 0; i<saltBalls.size(); i++){
						...
						if(saltBalls.get(i).getY() < ground.get(0).getY()){
							saltBalls.get(i).setY((int) saltBalls.get(i).getY() - 2);
						}
